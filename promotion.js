...
function checkPromotionEligibility() {
  ...
  const promoReqs = PROMOTION_REQUIREMENTS[exam]
  if (!promoReqs) return { eligible: false, reasons: [] }

  const req = promoReqs[profile.level]
  ...
  const boardRecords = records.filter(r => {
    const q = allQ.find(qq => qq.id === r.questionId)
...
function startChallenge() {
  ...
  const exam = localStorage.getItem('kuaiji_selected_exam') || 'junior'
  const examChallengeConfig = CHALLENGE_CONFIG[exam]
  if (!examChallengeConfig) return

  const config = examChallengeConfig[profile.level]
  if (!config) return

  const allQ = getExamQuestions()
  const pool = config.boards
    ? allQ.filter(q => config.boards.includes(q.category))
    : allQ
        ...
        <div class="card">
          ...
          ${st.answers.filter(a => !a.correct).map(a => {
            ...
            return `<div style="margin-bottom:16px">
              ...
              <div style="font-size:13px;color:var(--success)">正确答案：${correctOpt?.label}. ${correctOpt?.text}</div>
              <div style="font-size:12px;color:var(--text-secondary);margin-top:4px">${q.explanation}</div>
            </div>`
          }).join('')}
        </div>
